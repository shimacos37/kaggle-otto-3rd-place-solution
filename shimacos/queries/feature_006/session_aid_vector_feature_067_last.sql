...
{% for dataset_name in ['train_valid', 'train_test'] %}        
  ...
  n2v_feature AS (
    SELECT
      session,
      l.aid,
      dot_product(v1.vector, v2.vector) AS n2v_cosine_similarity,
      l2_distance(v1.vector, v2.vector) AS n2v_l2_distance
  ...
  w2v_feature AS (
    SELECT
      session,
      l.aid,
      dot_product(v1.vector, v2.vector) AS w2v_cosine_similarity,
      l2_distance(v1.vector, v2.vector) AS w2v_l2_distance
    FROM `{{ project_id }}.otto_067.{{ dataset_name }}_user_item` AS l
    LEFT JOIN last_aids USING (session)
  ...
  diff_feature AS (
    SELECT
      ...
      {% for distance in ['cosine_similarity', 'l2_distance',] %}
        w2v_{{ distance }} - avg_w2v_{{ distance }} AS diff_avg_w2v_{{distance}},
        w2v_{{ distance }} - min_w2v_{{ distance }} AS diff_min_w2v_{{distance}},
        w2v_{{ distance }} - max_w2v_{{ distance }} AS diff_max_w2v_{{distance}},
        n2v_{{ distance }} - avg_n2v_{{ distance }} AS diff_avg_n2v_{{distance}},
        n2v_{{ distance }} - min_n2v_{{ distance }} AS diff_min_n2v_{{distance}},
        n2v_{{ distance }} - max_n2v_{{ distance }} AS diff_max_n2v_{{distance}}{% endfor %}
    FROM joined
    LEFT JOIN (
      SELECT
        session,
        {% for distance in ['cosine_similarity', 'l2_distance'] %}
          avg(w2v_{{ distance }}) AS avg_w2v_{{distance}},
          avg(n2v_{{ distance }}) AS avg_n2v_{{distance}},
          min(w2v_{{ distance }}) AS min_w2v_{{distance}},
          min(n2v_{{ distance }}) AS min_n2v_{{distance}},
          max(w2v_{{ distance }}) AS max_w2v_{{distance}},
          max(n2v_{{ distance }}) AS max_n2v_{{distance}}{% endfor %}
      FROM joined
      GROUP BY 1
    ) USING (session)
  )
