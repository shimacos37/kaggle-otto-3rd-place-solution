...
{% for dataset_name in ['train_valid', 'train_test'] %}        
  ...
  n2v_feature AS (
    SELECT
      session,
      l.aid,
      dot_product(v1.vector, v2.vector) AS last_order_n2v_cosine_similarity,
      l2_distance(v1.vector, v2.vector) AS last_order_n2v_l2_distance
  ...
  w2v_feature AS (
    SELECT
      session,
      l.aid,
      dot_product(v1.vector, v2.vector) AS last_order_w2v_cosine_similarity,
      l2_distance(v1.vector, v2.vector) AS last_order_w2v_l2_distance
    FROM `{{ project_id }}.otto_067.{{ dataset_name }}_user_item` AS l
    LEFT JOIN last_aids USING (session)
  ...
  diff_feature AS (
    SELECT
      ...
      {% for distance in ['cosine_similarity', 'l2_distance',] %}
        last_order_w2v_{{ distance }} - avg_w2v_{{ distance }} AS diff_avg_last_order_w2v_{{distance}},
        last_order_w2v_{{ distance }} - min_w2v_{{ distance }} AS diff_min_last_order_w2v_{{distance}},
        last_order_w2v_{{ distance }} - max_w2v_{{ distance }} AS diff_max_last_order_w2v_{{distance}},
        last_order_n2v_{{ distance }} - avg_n2v_{{ distance }} AS diff_avg_last_order_n2v_{{distance}},
        last_order_n2v_{{ distance }} - min_n2v_{{ distance }} AS diff_min_last_order_n2v_{{distance}},
        last_order_n2v_{{ distance }} - max_n2v_{{ distance }} AS diff_max_last_order_n2v_{{distance}}{% endfor %}
    FROM joined
    LEFT JOIN (
      SELECT
        session,
        {% for distance in ['cosine_similarity', 'l2_distance'] %}
          avg(last_order_w2v_{{ distance }}) AS avg_w2v_{{distance}},
          avg(last_order_n2v_{{ distance }}) AS avg_n2v_{{distance}},
          min(last_order_w2v_{{ distance }}) AS min_w2v_{{distance}},
          min(last_order_n2v_{{ distance }}) AS min_n2v_{{distance}},
          max(last_order_w2v_{{ distance }}) AS max_w2v_{{distance}},
          max(last_order_n2v_{{ distance }}) AS max_n2v_{{distance}}{% endfor %}
      FROM joined
      GROUP BY 1
    ) USING (session)
  )
