...
{% endif %}
  ...
  pairs AS (
    ...
    WHERE
      a.rn != b.rn
      AND
      abs(timestamp_diff(a.ts, b.ts, HOUR)) <= 1
  ),
  ...
  WHERE
    rn_covisit_{{ sql_name }}_click <= 30
    OR rn_covisit_{{ sql_name }}_cart <= 30
    OR rn_covisit_{{ sql_name }}_order <= 30
    OR rn_covisit_{{ sql_name }}_aid_count <= 30
    OR rn_covisit_{{ sql_name }}_type_click <= 30
    OR rn_covisit_{{ sql_name }}_type_cart <= 30
    OR rn_covisit_{{ sql_name }}_type_order <= 30
    OR rn_covisit_{{ sql_name }}_type_aid_count <= 30
    OR rn_covisit_{{ sql_name }}_cvr_click_to_cart_order <= 30
    OR rn_covisit_{{ sql_name }}_cvr_cart_to_order <= 30;
