{% for dataset_name in ['train_valid', 'train_test'] %}    
  ...
  joined AS (
    SELECT
      * EXCEPT(
        click_label, click_labels, cart_label, order_label, ts_hour, action_min_ts, action_max_ts
      ),
    ...
    LEFT JOIN `{{ project_id }}.otto_feature_006.session_aid_feature_v2_{{ dataset_name }}` USING (session, aid)
