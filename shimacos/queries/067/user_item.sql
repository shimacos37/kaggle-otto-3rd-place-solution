{% for dataset_name in ['train_valid', 'train_test'] %}      
  ...
  FROM (
      ...
      SELECT DISTINCT
        session,
        aid
      FROM `{{ project_id }}.otto_preprocess_003.bpr_cosine_similarity_{{ dataset_name }}`,
        unnest(neighbor_aids) AS aid
      UNION ALL
      SELECT DISTINCT
        session,
        aid
      FROM `{{ project_id }}.otto_candidates_018.word2vec_min_count5_window5_sg_{{ dataset_name }}`
    )
  LEFT JOIN (
      SELECT DISTINCT
        session,
